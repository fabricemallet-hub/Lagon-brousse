...
export default function VesselTrackerPage() {
  ...
  const updateVesselInFirestore = useCallback(async (data: Partial<VesselStatus>) => {
    ...
    // Météo Windy (Quota 3h)
    if (payload.location) {
        const lastW = profile?.lastWeatherUpdate;
        const threeHours = 3 * 60 * 60 * 1000;
        const shouldUpdateWeather = !lastW || (Date.now() - (lastW.toMillis ? lastW.toMillis() : 0) > threeHours);
        
        if (shouldUpdateWeather) {
            const wData = await fetchWindyWeather(payload.location.latitude, payload.location.longitude);
            if (wData.success) {
                payload.windSpeed = wData.windSpeed;
                payload.windDir = wData.windDir;
                payload.wavesHeight = wData.waves;
                payload.lastWeatherUpdate = serverTimestamp();
            }
        }
    }
    ...
    setSyncCountdown(60);
  }, [user, firestore, isSharing, sharingId, vesselNickname, isGhostMode, mooringRadius, vesselStatus, profile?.lastWeatherUpdate]);
  ...
  useEffect(() => {
    ...
    followedVessels.forEach(vessel => {
        ...
        const batteryLevel = vessel.batteryLevel ?? 100;
        const battColor = batteryLevel > 60 ? 'text-green-500' : batteryLevel > 20 ? 'text-orange-500' : 'text-red-500';

        const iconHtml = `
            <div class="flex flex-col items-center gap-1">
                <div class="bg-white px-2 py-1 rounded shadow-2xl border-2 border-slate-200 text-[10px] font-black text-slate-800 whitespace-nowrap flex flex-col items-center">
                    <span>${vessel.displayName} | ${statusLabel}</span>
                    <span class="${battColor} text-[8px] flex items-center gap-1">
                        ${vessel.isCharging ? '⚡' : ''}${batteryLevel}%
                    </span>
                </div>
                <div class="p-2 rounded-full border-4 border-white shadow-2xl ${vessel.status === 'emergency' || vessel.status === 'drifting' ? 'animate-pulse' : ''}" style="background-color: ${color}">
                    <div class="text-white text-xl">${vessel.status === 'stationary' ? '⚓' : vessel.status === 'landed' ? '🏠' : '⛵'}</div>
                </div>
                ${vessel.windSpeed !== undefined ? `
                    <div class="bg-slate-900 text-white rounded-lg px-2 py-1 text-[8px] font-black shadow-xl mt-1 flex items-center gap-2 border border-white/10">
                        <span>💨 ${vessel.windSpeed}nd</span>
                        <span>🌊 ${vessel.wavesHeight}m</span>
                    </div>
                ` : ''}
            </div>
        `;
        ...
        if (!mapMarkersRef.current[vessel.id]) {
            ...
            if (vessel.status === 'stationary' || vessel.status === 'drifting') {
                ...
                map.removeLayer(entry.circle); entry.circle = undefined;
            }
        }
    });
  }, [map, followedVessels, mode, user, vesselPrefs.batteryThreshold]);
    ...
    <div className="flex flex-col gap-6 w-full max-w-full overflow-x-hidden px-1 pb-32">
      {loopingLabel && (
        <div className="fixed top-0 left-0 right-0 z-[300] bg-red-600 text-white p-4 flex items-center justify-between shadow-2xl animate-pulse">
            <div className="flex items-center gap-3"><AlertTriangle className="size-6" /><span className="font-black uppercase text-sm tracking-widest">{loopingLabel}</span></div>
      ...
      <header className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-black uppercase tracking-tighter flex items-center gap-2"><Globe className="text-primary" /> Boat Tracker</h1>
          ...
          {isSharing ? (
            <div className="space-y-4">
              <Card className="bg-primary text-white border-none shadow-xl overflow-hidden relative">
                ...
                <CardHeader className="p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge onClick={() => updateVesselInFirestore({ eventLabel: 'MAJ GPS FORCÉE' })} className="bg-white/20 border-white/20 text-white font-black text-[10px] h-6 px-3 cursor-pointer active:scale-95 transition-all">SYNC DANS {syncCountdown}S</Badge>
                  </div>
              ...
              <Card className="border-2 border-dashed p-4 space-y-4">
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-muted-foreground tracking-widest px-1"><HistoryIcon className="size-3" /> Signalisation manuelle</div>
                <div className="grid grid-cols-2 gap-2">
                    <Button variant={vesselStatus === 'returning' ? 'default' : 'outline'} className={cn("h-14 font-black uppercase text-[10px] border-2", vesselStatus === 'returning' && "bg-indigo-600 border-indigo-600")} onClick={() => handleManualStatus('returning', 'RETOUR MAISON')}><Navigation className="mr-2 size-4" /> {vesselStatus === 'returning' ? 'ANNULER RETOUR' : 'RETOUR MAISON'}</Button>
                    <Button variant={vesselStatus === 'landed' ? 'default' : 'outline'} className={cn("h-14 font-black uppercase text-[10px] border-2", vesselStatus === 'landed' && "bg-green-600 border-green-600")} onClick={() => handleManualStatus('landed', 'HOME (À TERRE)')}><Home className="mr-2 size-4" /> {vesselStatus === 'landed' ? 'ANNULER HOME' : 'HOME (À TERRE)'}</Button>
                </div>
              </Card>

              <Card className="border-2 border-dashed p-4 space-y-4">
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-muted-foreground tracking-widest px-1"><Target className="size-3" /> Signalement Tactique</div>
              ...
              <Button variant={vesselStatus === 'emergency' ? 'destructive' : 'secondary'} className={cn("w-full h-16 text-xs font-black uppercase tracking-widest shadow-lg rounded-xl gap-3 border-2", vesselStatus === 'emergency' && "animate-pulse border-white/20")} onClick={() => handleManualStatus('emergency', 'DEMANDE ASSISTANCE')}><ShieldAlert className="size-6" /> {vesselStatus === 'emergency' ? 'ANNULER ALERTE' : "DEMANDE D'ASSISTANCE"}</Button>
              <Button variant="destructive" className="w-full h-14 font-black uppercase text-xs tracking-widest opacity-80" onClick={handleStopSharing}><X className="mr-2 size-4" /> ARRÊTER LE PARTAGE / QUITTER</Button>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="p-4 bg-muted/20 rounded-2xl border-2 border-dashed space-y-4">
                <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase ml-1 opacity-60">Surnom du capitaine / navire</Label><Input value={vesselNickname} onChange={e => setVesselNickname(e.target.value)} placeholder="Ex: Koolapik" className="h-12 border-2 font-bold" /></div>
      ...
      <div className={cn("relative w-full transition-all bg-slate-900 rounded-[2.5rem] border-4 border-slate-800 shadow-2xl overflow-hidden", isFullscreen ? "fixed inset-0 z-[150] h-screen w-screen rounded-none" : "h-[550px]")}>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-[160]">
            <Button size="icon" className="bg-white/90 backdrop-blur-md border-2 h-10 w-10 shadow-xl" onClick={() => setIsFullscreen(!isFullscreen)}>{isFullscreen ? <Shrink className="size-5 text-primary" /> : <Expand className="size-5 text-primary" />}</Button>
            <Button size="icon" className={cn("bg-white/90 backdrop-blur-md border-2 h-10 w-10 shadow-xl", isFollowMode && "bg-blue-50 border-blue-500 animate-pulse")} onClick={() => setIsFollowMode(!isFollowMode)}><Navigation className={cn("size-5", isFollowMode ? "text-blue-600" : "text-primary")} /></Button>
            <Button onClick={handleRecenter} className="h-10 bg-white/90 backdrop-blur-md border-2 px-3 gap-2 shadow-xl font-black uppercase text-[9px] text-primary whitespace-nowrap">RECENTRER <LocateFixed className="size-4" /></Button>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="id-ids" className="border-2 rounded-xl bg-muted/10 overflow-hidden mb-2">
                <AccordionTrigger className="px-4 py-3 hover:no-underline font-black uppercase text-[10px] tracking-widest"><div className="flex items-center gap-2"><Settings className="size-4 text-primary" /> Identité & IDs</div></AccordionTrigger>
                <AccordionContent className="p-4 space-y-4">
                    <div className="space-y-4">
                        <div className="space-y-1.5"><div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black uppercase opacity-60">Rayon de mouillage</Label><Badge variant="outline" className="font-black bg-white">{mooringRadius}m</Badge></div><Slider value={[mooringRadius]} min={10} max={200} step={5} onValueChange={v => setMooringRadius(v[0])} /></div>
                        <div className="flex items-center justify-between p-3 border rounded-xl bg-white"><div className="space-y-0.5"><Label className="text-xs font-black uppercase">Mode Fantôme</Label><p className="text-[8px] font-bold text-muted-foreground uppercase">Masquer pour la flotte (C)</p></div><Switch checked={isGhostMode} onCheckedChange={setIsGhostMode} /></div>
                    </div>
                </AccordionContent>
            </AccordionItem>

            <AccordionItem value="sms-settings" className="border-2 rounded-xl bg-muted/10 overflow-hidden mb-2">
                <AccordionTrigger className="px-4 py-3 hover:no-underline font-black uppercase text-[10px] tracking-widest"><div className="flex items-center gap-2"><Smartphone className="size-4 text-orange-600" /> Réglages SMS d'Urgence</div></AccordionTrigger>
                <AccordionContent className="p-4 space-y-4">
                    <div className="space-y-4">
            ...
            <AccordionItem value="tech-log" className="border-2 rounded-xl bg-muted/10 overflow-hidden mb-2">
                <AccordionTrigger className="px-4 py-3 hover:no-underline font-black uppercase text-[10px] tracking-widest"><div className="flex items-center gap-2"><HistoryIcon className="size-4 text-primary" /> Journal Technique</div></AccordionTrigger>
                <AccordionContent className="p-0 border-t border-dashed">
                    <ScrollArea className="h-64">
                        <div className="divide-y">
                            {history.map((h, i) => (
                                <div key={i} className="p-3 flex items-center justify-between text-[10px] bg-white">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2"><span className="font-black text-blue-600">{h.vesselName}</span><span className="font-black uppercase text-slate-800">{h.statusLabel}</span></div>
                                        <div className="flex items-center gap-2 text-slate-400 font-bold mt-0.5"><span>{format(h.time, 'HH:mm:ss')}</span></div>
                                    </div>
                            ...
                            {history.length === 0 && <p className="text-center py-10 text-xs italic opacity-40">Aucun événement enregistré.</p>}
                        </div>
                    </ScrollArea>
                </AccordionContent>
            </AccordionItem>
      ...
      <Card className="border-2 bg-muted/10 shadow-none rounded-2xl overflow-hidden">
        <CardHeader className="p-4 pb-2 border-b bg-muted/5"><CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 text-muted-foreground"><Phone className="size-4 text-primary" /> Annuaire Maritime NC</CardTitle></CardHeader>
        <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="space-y-3"><h4 className="text-[10px] font-black uppercase text-indigo-600 flex items-center gap-2 border-b pb-1"><Ship className="size-3" /> Ports & Marinas</h4><div className="space-y-2"><a href="tel:255000" className="flex flex-col group"><span className="text-[9px] font-bold text-muted-foreground uppercase">Port Autonome (VHF 12)</span><span className="text-sm font-black group-hover:text-indigo-600 transition-colors">25 50 00</span></a></div></div>
        </CardContent>
      </Card>
    </div>
  );
