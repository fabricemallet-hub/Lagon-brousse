...
export default function VesselTrackerPage() {
  ...
  const updateVesselInFirestore = useCallback((data: Partial<VesselStatus>) => {
    ...
    const update = async () => {
        ...
        } else if (!updatePayload.location && currentPosRef.current) {
  ...
  const handleToggleHidePosition = useCallback(() => {
    const nextHidden = !isPositionHidden;
    setIsPositionHidden(nextHidden);
    updateVesselInFirestore({ isPositionHidden: nextHidden });
    toast({ title: nextHidden ? "Mode Fantôme : Position masquée" : "Position de nouveau visible" });
  }, [isPositionHidden, updateVesselInFirestore, toast]);
  ...
  useEffect(() => {
    ...
    followedVessels.forEach(vessel => {
        ...
        const pos = vessel.isPositionHidden 
            ? null 
            : { lat: vessel.location?.latitude || INITIAL_CENTER.lat, lng: vessel.location?.longitude || INITIAL_CENTER.lng };
        ...
        let label = currentEvent || statusLabels[currentStatus] || currentStatus;
        if (vessel.isPositionHidden && currentStatus !== 'offline') {
            label += ' (POS. MASQUÉE)';
        }
        ...
        if (mode === 'sender') {
            ...
            if (isFirstFixRef.current) {
                ...
                updateVesselInFirestore({ 
                    location: isPositionHidden ? null : { latitude: newPos.lat, longitude: newPos.lng }, 
                    status: 'moving', 
                    isSharing: true,
  ...
  }, [isSharing, isReceiverGpsActive, mode, updateVesselInFirestore, map, toast, isPositionHidden]);
              ...
              {isSharing ? (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="bg-muted/20 p-4 rounded-2xl border-2 border-dashed space-y-3">
                        <div className="flex items-center justify-between p-3 bg-background rounded-xl border mb-2 shadow-sm">
                            <div className="space-y-0.5">
                                <Label className="text-xs font-black uppercase flex items-center gap-2">
                                    {isPositionHidden ? <EyeOff className="size-4 text-orange-600" /> : <Eye className="size-4 text-primary" />}
                                    Mode Fantôme
                                </Label>
                                <p className="text-[8px] font-bold text-muted-foreground uppercase">Masquer la position GPS au récepteur</p>
                            </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid gap-2">
                        {savedVesselIds.map(id => {
                            ...
                            const isHidden = vessel?.isPositionHidden === true;
                            return (
                                <div key={id} className={cn("flex items-center justify-between p-3 border-2 rounded-xl transition-all shadow-sm cursor-pointer", vessel?.status === 'emergency' ? "bg-red-50 border-red-500 animate-pulse" : isActive ? "bg-primary/5 border-primary/20" : "bg-muted/5 opacity-60")} onClick={() => { if (isActive && !isHidden && vessel?.location && map) { map.panTo({ lat: vessel.location.latitude, lng: vessel.location.longitude }); map.setZoom(15); } }}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className={cn("text-[8px] font-bold uppercase", vessel?.status === 'emergency' ? 'URGENCE' : isActive ? 'En ligne' : 'Off')}>
                                                {vessel?.status === 'emergency' ? 'URGENCE' : isActive ? (isHidden ? 'EN LIGNE (MASQUÉ)' : 'EN LIGNE') : 'OFF'}
                                            </span>
                                        </div>
                                    </div>
                ...
                <AccordionItem value="receiver-settings" className="border-none">
                  ...
                  <AccordionContent className="pt-4 space-y-6">
                    <div className="space-y-4 p-4 border-2 rounded-2xl bg-card shadow-inner">
                      ...
                      <div className={cn("space-y-4 transition-opacity", !vesselPrefs.isNotifyEnabled && "opacity-40 pointer-events-none")}>
                        ...
                        <div className="space-y-4 pt-4 border-t border-dashed">
                          <p className="text-[9px] font-black uppercase text-muted-foreground">Personnalisation des sons</p>
                          <div className="grid gap-3">
                            {[
                              { key: 'moving', label: 'Mouvement' },
                              { key: 'stationary', label: 'Mouillage' },
                              { key: 'offline', label: 'Signal Perdu' },
                              { key: 'emergency', label: 'Assistance' },
                              ...
                              <div key={item.key} className="flex items-center justify-between gap-4">
                                <div className="flex flex-col flex-1">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        />
                                        <span className="text-[8px] font-black uppercase opacity-40">{vesselPrefs.notifySettings[item.key as keyof typeof vesselPrefs.notifySettings] ? 'Actif' : 'Off'}</span>
                                    </div>
                                </div>
                                <Select 
                    ...
                    <div className="space-y-4 p-4 border-2 rounded-2xl bg-orange-50/30 border-orange-100">
                        <div className="flex items-center justify-between">
                            <div className="space-y-0.5">
                                <Label className="text-xs font-black uppercase text-orange-800">Veille Stratégique (Immobilité)</Label>
                        ...
                        <div className={cn("space-y-4 pt-2 border-t border-orange-100", !vesselPrefs.isWatchEnabled && "opacity-40 pointer-events-none")}>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Label className="text-[9px] font-black uppercase opacity-60">Durée</Label>
                                    <Select 
                                        value={vesselPrefs.watchDuration.toString()} 
                                        onValueChange={v => saveVesselPrefs({ ...vesselPrefs, watchDuration: parseInt(v) })}
                                    >
                                        <SelectTrigger className="h-9 text-[10px] font-black uppercase bg-white"><SelectValue /></SelectTrigger>
                                        <SelectContent>
                                ...
                                <div className="space-y-1.5">
                                    <Label className="text-[9px] font-black uppercase opacity-60">Son d'alarme</Label>
                                    <div className="flex gap-1">
                                        <Select 
                                            value={vesselPrefs.watchSound} 
                                            onValueChange={v => saveVesselPrefs({ ...vesselPrefs, watchSound: v })}
                                        >
                                            <SelectTrigger className="h-9 text-[10px] font-black uppercase bg-white flex-1"><SelectValue /></SelectTrigger>
                                            <SelectContent>
                                                {availableSounds.map(s => <SelectItem key={s.id} value={s.id} className="text-[9px] uppercase font-black">{s.label}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                        <Button variant="ghost" size="icon" className="h-9 w-9 bg-white" onClick={() => playVesselSound(vesselPrefs.watchSound)}><Play className="size-3" /></Button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4 p-4 border-2 rounded-2xl bg-red-50/30 border-red-100">
                        <div className="flex items-center justify-between">
                            <div className="space-y-0.5">
                                <Label className="text-xs font-black uppercase text-red-800">Batterie Faible</Label>
                        ...
                        <div className="flex gap-4 items-center">
                            ...
                            <div className="flex gap-1 items-center">
                                ...
                                >
                                    <SelectTrigger className="h-8 text-[9px] font-black uppercase w-24 bg-white"><SelectValue placeholder="Son..." /></SelectTrigger>
                                    <SelectContent>
                                        {availableSounds.map(s => <SelectItem key={s.id} value={s.id} className="text-[9px] uppercase font-black">{s.label}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                                <Button variant="ghost" size="icon" className="h-8 w-8 bg-white" onClick={() => playVesselSound(vesselPrefs.batterySound)}><Play className="size-3" /></Button>
                            </div>
                        </div>
                    </div>
                  </AccordionContent>
          ...
          <GoogleMap mapContainerClassName="w-full h-full" defaultCenter={INITIAL_CENTER} defaultZoom={10} onLoad={setMap} options={{ disableDefaultUI: true, mapTypeId: 'satellite', gestureHandling: 'greedy' }}>
                {followedVessels?.map(vessel => (
                    <React.Fragment key={`vessel-markers-${vessel.id}`}>
                        {vessel.isSharing && !vessel.isPositionHidden && vessel.location && (
                                            ...
                                            <div className="flex items-center justify-between gap-4">
                                                <div className="flex flex-col gap-1 min-w-0 flex-1">
                                                    <span className={cn(
                                                        ...
                                                        h.statusLabel.includes('POSITION') ? 'text-green-600' : 
                                                        h.statusLabel.includes('MASQUÉE') ? 'text-orange-600' : 
                                                        h.statusLabel.includes('MOUILLAGE') ? 'text-orange-500' : 'text-slate-600'
                                                    )}>
                                                        {h.statusLabel}
                                                    </span>
