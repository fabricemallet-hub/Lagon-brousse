...
/**
 * MARQUEUR NAVIRE MÉMOÏSÉ v124.0
 * Empêche le re-render si la position ou l'état ne change pas.
 */
const VesselMarker = React.memo(({ vessel, isRollingBack }: { vessel: VesselStatus, isRollingBack?: boolean }) => {
    ...
    // Optimisation : On ne re-render que si le bateau bouge de plus de 5m ou change de statut
    if (prev.isRollingBack !== next.isRollingBack) return false;
    if (prev.vessel.status !== next.vessel.status) return false;
    if (prev.vessel.isSharing !== next.vessel.isSharing) return false;
    
    const d = getDistance(
        prev.vessel.location?.latitude || 0, prev.vessel.location?.longitude || 0,
        next.vessel.location?.latitude || 0, next.vessel.location?.longitude || 0
    );
    return d < 5;
});
...
export default function VesselTrackerPage() {
  ...
  }, [emetteur.currentPos, simulator.simPos, mapCore.isMapReady]);
  ...
  const handleTactical = useCallback((type: string) => {
    ...
    if (pos) {
        ...
        toast({ variant: "destructive", title: "GPS Inactif" });
    }
  ...
  const sendEmergencySms = (type: 'SOS' | 'MAYDAY' | 'PAN PAN' | 'ASSISTANCE') => {
    if (!emetteur.isEmergencyEnabled) {
        toast({ variant: "destructive", title: "Service désactivé" });
        return;
    }
    ...
    const posUrl = pos ? `https://www.google.com/maps?q=${pos.lat.toFixed(6)},${pos.lng.toFixed(6)}` : "[GPS...]";
    const body = `[${(emetteur.vesselNickname || 'NAV').toUpperCase()}] ${emetteur.vesselSmsMessage || 'Besoin assistance.'} [${type}] Pos: ${posUrl}`;
    window.location.href = `sms:${contact.replace(/\s/g, '')}${/iPhone|iPad|iPod/.test(navigator.userAgent) ? '&' : '?'}body=${encodeURIComponent(body)}`;
  };

  /**
   * CONTENEUR CARTE MÉMOÏSÉ v124.0
   * Isole l'instance Google Map des mises à jour textuelles (HUD, Logs).
   */
  ...
  }, [isLoaded, mapCore.breadcrumbs, mapCore.tacticalMarkers, emetteur.isTrajectoryHidden, activeAnchorVessel, mooringCircleOptions, followedVessels, emetteur.isSharing, simulator.isActive, emetteur.currentPos, simulator.simPos, emetteur.isRollingBack]);
  ...
  return (
    <div className="w-full space-y-4 pb-32 px-1 relative">
        ...
        <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-2 pointer-events-auto">
            ...
            <div className="mt-4 flex flex-col gap-2">
                <Button 
                    ...
                    onClick={() => mapCore.setWindyLayer('none')}
                >
                ...
                {[
                    ...
                    <Button 
                        ...
                        onClick={() => mapCore.setWindyLayer(layer.id as any)}
                    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 relative z-[10000]">
          <Button 
            variant="destructive" 
        ...
        <TabsContent value="sender">
            {emetteur.isSharing ? (
                <div className="space-y-4">
                    <Card className={cn("p-6 rounded-2xl shadow-xl relative overflow-hidden border-2", emetteur.vesselStatus === 'landed' ? "bg-green-600 border-green-400/20" : "bg-primary border-primary-foreground/20")}>
                        ...
                        <div className="space-y-1 relative z-10 text-white">
                            <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><Zap className="size-3 fill-yellow-300 text-yellow-300" /> Partage en cours</p>
                    ...
                    <CardContent className="p-6 space-y-6">
                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <Input value={emetteur.vesselNickname} onChange={e => emetteur.setVesselNickname(e.target.value)} placeholder="KOolapik" className="h-14 border-2 font-black text-lg bg-slate-100" />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5">
                                    <Label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">ID Navire</Label>
                                    <Input value={emetteur.customSharingId} onChange={e => emetteur.setCustomSharingId(e.target.value)} placeholder="XXX" className="h-14 border-2 font-black text-center text-sm uppercase bg-slate-100" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-[10px] font-black uppercase text-[#8b5cf6] tracking-widest ml-1">ID Flotte</Label>
                                    <Input value={emetteur.customFleetId} onChange={e => emetteur.setCustomFleetId(e.target.value)} placeholder="ABC" className="h-14 border-2 font-black text-center text-sm uppercase bg-[#f5f3ff]" />
                                </div>
                            </div>
                            <Button onClick={emetteur.startSharing} className="w-full h-16 font-black uppercase tracking-widest text-sm shadow-xl gap-3 rounded-2xl bg-primary">
          ...
          <AccordionItem value="cockpit" className="border-none">
              ...
              <AccordionContent className="pt-4 space-y-4">
                  <Tabs defaultValue="tactical" className="w-full">
                          ...
                          <div className="grid grid-cols-4 gap-2">
                              {['MARLIN', 'THON', 'TAZARD', 'WAHOO', 'BONITE', 'SARDINES', 'OISEAUX', 'PRISE'].map(type => (
                                  <Button key={type} variant="outline" className="flex flex-col items-center justify-center h-20 rounded-xl border-2 gap-1 touch-manipulation" onClick={() => handleTactical(type)}>
                                      {type === 'OISEAUX' ? <Bird className="size-5 text-primary" /> : type === 'PRISE' ? <Camera className="size-5 text-primary" /> : <Fish className="size-5 text-primary" />}
                      ...
                      <TabsContent value="settings" className="m-0 p-0 bg-slate-50">
                          ...
                          <ScrollArea className="h-[400px]">
                            <div className="p-4 space-y-6 pb-10">
                                <Card className="border-2 border-primary/10 bg-primary/5 rounded-2xl p-4 flex items-center justify-between">
                                    <Label className="text-11px] font-black uppercase text-primary">Alertes Sonores</Label>
                                ...
                                <div className="grid gap-4 pt-4 border-t border-dashed">
                                    {['moving', 'stationary', 'drifting', 'offline', 'assistance', 'tactical', 'battery'].map(key => {
                                        ...
                                        return (
                                            <Card key={key} className="border-2 rounded-2xl overflow-hidden shadow-sm bg-white p-3 flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <Select value={config.sound} onValueChange={(v) => {
                                                        ...
                                                        <SelectTrigger className="h-10 border-2 font-black uppercase text-[10px] w-32"><SelectValue /></SelectTrigger>
                                                        <SelectContent>{recepteur.availableSounds.map(s => <SelectItem key={s.id} value={s.label}>{s.label}</SelectItem>)}</SelectContent>
                                                    </Select>
                                                    <Switch checked={config.enabled} onCheckedChange={(v) => {
                                                        const newAlerts = { ...recepteur.vesselPrefs.alerts };
                      ...
                      <TabsContent value="sms" className="m-0 p-0 bg-slate-50">
                          <ScrollArea className="h-[500px]">
                              <div className="p-4 space-y-6 pb-10">
                                <Card className="border-2 shadow-lg overflow-hidden rounded-3xl bg-white p-5 space-y-6">
                                    <div className="space-y-1.5">
                                        <Label className="text-[9px] font-black uppercase text-slate-500">NUMÉRO D'URGENCE (SOS)</Label>
                                        <Input value={emetteur.emergencyContact} onChange={e => emetteur.setEmergencyContact(e.target.value)} placeholder="77..." className="h-12 border-2 font-black text-lg" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="text-[9px] font-black uppercase text-slate-500">NUMÉRO ASSISTANCE</Label>
                                        <Input value={emetteur.assistanceContact} onChange={e => emetteur.setAssistanceContact(e.target.value)} placeholder="77..." className="h-12 border-2 font-black text-lg" />
                                    </div>
                                    <Button onClick={emetteur.handleSaveSmsSettings} className="w-full h-14 bg-primary text-white font-black uppercase text-[11px] tracking-widest rounded-2xl gap-3">
                        ...
                        <TabsContent value="labo" className="m-0 p-4 space-y-6 bg-white">
                            <div className="p-6 border-2 border-dashed border-red-200 rounded-[2.5rem] bg-red-50/20 space-y-8">
                                <div className="flex items-center justify-between">
                                    <Label className="text-sm font-black uppercase text-red-600">Sandbox Tactique</Label>
                                ...
                                <div className={cn("space-y-8", !simulator.isActive && "opacity-40 pointer-events-none")}>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                                        <div className="space-y-3">
                                            <div className="flex justify-between px-1"><Label className="text-[10px] font-black uppercase">Vitesse</Label><span className="text-xs font-black text-red-600">{simulator.simSpeed.toFixed(1)} ND</span></div>
                                            <Slider value={[simulator.simSpeed]} max={40} step={0.1} onValueChange={v => simulator.setSimSpeed(v[0])} />
                                        </div>
                                        <div className="space-y-3">
                                            <div className="flex justify-between px-1"><Label className="text-[10px] font-black uppercase">Batterie</Label><span className="text-xs font-black text-red-600">{simulator.simBattery}%</span></div>
                                            <Slider value={[simulator.simBattery]} max={100} step={1} onValueChange={v => simulator.setSimBattery(v[0])} />
                                        </div>
                                    </div>
                                    <Button className={cn("w-full h-20 text-lg font-black uppercase tracking-widest shadow-2xl rounded-3xl", simulator.isMoving ? "bg-red-600" : "bg-primary")} onClick={() => simulator.setIsMoving(!simulator.isMoving)}>
